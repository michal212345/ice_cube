...
from ice_cube import root_folder, github_url, bl_info
...
from ice_cube_data.utils.general_func import GetListIndex, BlenderVersConvert
...
def open_user_packs(self, context):
    #addon location
    try:
        asset_directory = os.path.join(root_folder,"ice_cube_data","internal_files","user_packs")
        os.startfile(asset_directory) # This should natively open the directory, regardless of OS
    except Exception as e:
        CustomErrorBox("Error Opening Directory, check console",'ERROR')
...
def install_update_func(self, context):
    #sets up variables
    install_loc = os.path.join(root_folder, "")
    download_folder = os.path.join(root_folder, "downloads")
    backups_folders = os.path.join(root_folder, "backups")
    can_backup = False
    ...
    for file in getFiles(backups_folders):
        creation_date = pathlib.Path(os.path.join(backups_folders, file)).stat().st_mtime
    ...
    download_file_loc = os.path.join(download_folder, "latest_release.zip")
    ...
    for file in getFiles(download_folder):
        os.rename(os.path.join(download_folder, file), os.path.join(download_folder, "Ice Cube"))
    
    #Install the downloaded addon
    try:
        distutils.dir_util.copy_tree(os.path.join(download_folder, "Ice Cube"), install_loc)
...
def create_backup_func(self, context):
    #sets up variables
    obj = context.object
    virtual_ice_cube = os.path.join(root_folder, "Ice Cube")
    ...
    if obj.get("backup_name"):
        if obj.get("backup_name") == "":
            backups_folder = os.path.join(os.path.dirname(backups_folder), "backups", "main")
            if os.path.exists(backups_folder) is False:
                os.mkdir(backups_folder)
        else:
            backups_folder = os.path.join(os.path.dirname(backups_folder), "backups", backup_name)
            if os.path.exists(backups_folder) is False:
                os.mkdir(backups_folder)
    else:
        backups_folder = os.path.join(os.path.dirname(backups_folder), "backups", "main")
    ...
    try:
        for file in files_nopath:
            ...
            shutil.copy(os.path.join(virtual_ice_cube, file), os.path.join(backups_folder, files_nopath[GetListIndex(file_nopy, files_nopath)]))

        for folder in folders_nopath:
            try:
                os.mkdir(os.path.join(backups_folder, folders_nopath[GetListIndex(folder, folders_nopath)]))
            except:
                pass
            
        for folder in folders_nopath:
            distutils.dir_util.copy_tree(os.path.join(virtual_ice_cube, folder), os.path.join(backups_folder, folder))
        if obj.get("backup_name"):
            if obj.get("backup_name") == "":
                backup_name = "main"
            else:
...
def refresh_backups_func(self, context):
    ...
    for backup in backup_folder_scan:
        try:
            ...
            creation_date = pathlib.Path(os.path.join(backups_folder, backup)).stat().st_mtime
...
def load_backup_func(self, context):
    ...
    backup_loc = os.path.join(root_folder, "backups", selected_backup)
...
def delete_backup_func(self, context):
        ...
        backup_to_remove = os.path.join(os.path.dirname(backups_folder), "backups", selected_backup)
...
def IC_download_dlc(self, context, valid_dlcs):
    ...
    try:
        ...
        download_file_loc = os.path.join(download_folder, "dlc.zip")
...
def export_settings_data(self, context):
    ...
    if obj.get("prop_clipboard") == True:
        ...
        if filename == "":
            ...
            return{'FINISHED'}
        with open(os.path.join(filepath, f"{filename}.ice_cube_data"), "w") as json_file:
...
def reset_all_settings_func(self, context):
    ...
    settings_json_data = open_json(os.path.join(root_folder, "ice_cube_data", "internal_files", "default_settings.ice_cube_data"))
...
def generate_settings_json():
    ...
    settings_json_path = os.path.join(root_folder, "ice_cube_data", "settings.json")
